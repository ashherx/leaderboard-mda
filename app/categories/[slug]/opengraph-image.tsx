...
export default async function CategoryOgImage({
  ...
  const pricing = category
    ? await getCategoryPricing(category.id, category.min_bid_cents)
    : null;
  ...
  return new ImageResponse(
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        />
        <span style={{ fontSize: 28, color: SLATE, letterSpacing: 1 }}>
          THE PODIUM
        </span>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column" }}>
        <span
          style={{ fontSize: 72, fontWeight: 700, color: INK, lineHeight: 1.1 }}
        >
