...
// A manual route handler, not the opengraph-image.tsx file convention -
// that convention only varies by route params, and category selection now
// lives in a query string (?category=) since browsing collapsed onto one
// URL. generateMetadata in app/page.tsx points openGraph.images at this
// route with the category baked into the querystring instead.
...
export async function GET(request: Request) {
  const slug = new URL(request.url).searchParams.get("category");
    ...
    (
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontSize: 28, color: SLATE, letterSpacing: 1 }}>THE PODIUM</span>
        </div>

        <div style={{ display: "flex", flexDirection: "column" }}>
          <span style={{ fontSize: 72, fontWeight: 700, color: INK, lineHeight: 1.1 }}>
            {category?.name ?? "The Podium"}
          </span>
          <span style={{ fontSize: 32, color: SLATE, marginTop: 16 }}>Ranked purely by who&apos;s paid the most</span>
        </div>
