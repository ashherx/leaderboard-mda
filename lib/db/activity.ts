...
export async function recordClickEvent(listingId: string, categoryId: string, locationId: string): Promise<void> {
  const supabase = getSupabaseServerClient();
  const { error } = await supabase
    .from("click_events")
    .insert({ listing_id: listingId, category_id: categoryId, location_id: locationId });
...
export async function getTrendingListings(categoryId: string, locationId: string): Promise<TrendingListing[]> {
  const supabase = getSupabaseServerClient();
 ...
 * panel.
 */
export async function getLatestActivity(categoryId: string, locationId: string): Promise<ActivityItem[]> {
  const supabase = getSupabaseServerClient();
